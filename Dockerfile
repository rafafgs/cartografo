# The official image of the control plane (D23, t250; D27, t549).
#
# D23 says the control plane has an image and the runner does not: the runner
# needs the authenticated engine CLI and the target repository on the machine it
# runs on, and putting it here would only mean carrying both of those in too. So
# this image ships the single `cartografo` package — every command, one tarball
# (t248) — and `compose.yml` runs one container from it, the control plane.
# `cartografo-runner` stays a command an operator types on the host, pointed at
...
# (`scripts/link-bundled-siblings.mjs`, which is what puts the four siblings
...
# Deliberately no `ENV CARTOGRAFO_HOST` anywhere above: the image inherits the
# loopback default the code itself chooses (`packages/core/src/index.ts`), for
...
# `EXPOSE` documents the port; it opens nothing by itself.
EXPOSE 4317
...
# The control plane alone — the same flag `pack-install.e2e.test.ts` calls
# `CONTROL_PLANE_ONLY`. `CMD` and not `ENTRYPOINT`, so an operator who wants
# another of the package's commands from this image replaces the whole command
# with one `command:` key or one argument to `docker run`.
CMD ["cartografo", "--no-runner"]
