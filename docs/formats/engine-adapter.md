...
> - **`SessionSpec.command`** (2026-08-31) — the argv a node RUNS, for the
...
export interface SessionSpec {
  ...
  /**
   * The command this node RUNS, when the node runs a command instead of a
   * model.
   *
...
what grew after the v1 freeze (items 5 to 11). Nothing here is decorative: items 1, 3, 6, 7,
...
11. **A node that runs a command instead of a model** (2026-08-31). The
