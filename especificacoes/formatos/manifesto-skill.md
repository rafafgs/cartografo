...
não bate com o do registro, e renderiza `instrucoes`, `checks` e `permissoes`
para dentro da sessão
([`render-skill-instructions.ts`](../../packages/runner/src/dispatch/render-skill-instructions.ts)).
Ainda não implementado: a interpolação de `{{entrada.<caminho>}}` dentro de
`instrucoes`, que renderiza literal (ver *Limites conhecidos*); ler
`orcamentos` para dentro do despacho; e reimportar/versionar uma skill já
...
não existe" — a `t161` o construiu, e `instrucoes`, `checks` e `permissoes` já
atravessam (ver *Renderização e injeção*) —, e passou a ser simplesmente que
`orcamentos` ficou de fora daquela ficha. O que existe é o contrato aqui, o
...
- **Interpolação.** A convenção `{{entrada.<caminho>}}` e a regra de falha
  ...
  renderiza `instrucoes` **literalmente** — um `{{...}}` não resolvido chega à
