...
--
-- - `motor` é o nome que o adapter dá a si mesmo (`EngineAdapter.engineName`),
...
-- - `modelo_id` é o identificador que vai depois de `--model`/`-m`. Texto
...
-- - `rotulo` é o nome legível, quando o adapter tem um. NULO é normal.
-- - `origem` diz de onde a linha veio: `cli` (o adapter perguntou ao binário) ou
...
-- - `atualizado_em` é quando o relato chegou. Sem ele não dá para dizer se um
--   catálogo é de agora ou de um runner que morreu semana passada.
--
-- **Relato substitui, nunca soma.** O índice único `(motor, modelo_id)` é o que
...
--
-- Nomes de coluna seguem em português, como `runner`, `lease`, `sessao` e
-- `proposta`: a D18 escopa a regra de inglês a identificadores de código, e
-- schema aqui é vocabulário de dado. Os dois VALORES de `origem` são inglês por
-- serem vocabulário da interface do EngineAdapter, que é inglesa inteira e é
-- quem os produz — o mesmo raciocínio que a 0011 escreveu para `wall_clock` e
-- `silence` de `timeout_reason`.
--
-- Nenhuma migração abre transação própria: quem transaciona é src/db/migrate.ts.

CREATE TABLE motor_modelo (
  id            INTEGER PRIMARY KEY AUTOINCREMENT,
  motor         TEXT NOT NULL,             -- nome que o adapter dá a si mesmo
  modelo_id     TEXT NOT NULL,             -- o que vai depois de --model / -m
  rotulo        TEXT,                      -- NULO = o adapter não deu nome legível
  origem        TEXT NOT NULL
                  CHECK (origem IN ('cli', 'catalog')),
...
CREATE UNIQUE INDEX idx_motor_modelo_unico ON motor_modelo (motor, modelo_id);
