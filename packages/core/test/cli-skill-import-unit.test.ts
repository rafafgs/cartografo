...
function skillSource(frontmatter: string, body = 'Faça o trabalho.\n'): string {
  return `---\n${frontmatter}\n---\n\n${body}`;
}
...
test('the frontmatter is read flat, and a malformed one is refused loudly', () => {
  ...
  assert.deepEqual(read.frontmatter, { name: 'uma-skill', description: 'com aspas' });
  assert.equal(read.body, 'O corpo.\n', 'the blank lines after the block belong to the block');

  assert.throws(() => splitFrontmatter('# Sem frontmatter\n'), UsageError);
  assert.throws(() => splitFrontmatter('---\nname: uma-skill\n'), UsageError);
});

test('kebabCase strips accents, punctuation and the dashes it would end on', () => {
  assert.equal(kebabCase('Revisão de Código'), 'revisao-de-codigo');
  assert.equal(kebabCase('  --npm run test!!  '), 'npm-run-test');
  assert.equal(kebabCase('já-assim'), 'ja-assim');
...
test('only a command quoted inside a fenced block becomes a check', () => {
  const checks = deriveChecks(
    [
      'Rode a suíte (`npm test`) e confirme as falhas.',
      '',
      '```bash',
      '# um comentário não é comando',
      '',
      'npm test',
      'npm test!',
      'npm test',
      'git commit -m "nada disso"',
      'cd /tmp',
      'pytest -q',
      '```',
      '',
  ...
  assert.deepEqual(deriveChecks('sem fence nenhuma\n'), []);
});
...
test('a role that is neither work nor gate never becomes a draft', async (t) => {
  ...
  const source = sourceFile(t, skillSource('name: uma-skill'));
...
test('a source with no usable name is refused before the registry is asked', async (t) => {
  ...
  await assert.rejects(
    capture(() => runScanSkill(scanOptions(sourceFile(t, skillSource('description: sem nome')), plane.url))),
...
test('a source that cannot be read is a message, not a stack trace', async (t) => {
  ...
  await assert.rejects(
    capture(() => runScanSkill(scanOptions('/nao/existe/SKILL.md', plane.url))),
...
test('an id already registered is a human decision, not an overwrite', async (t) => {
  const plane = await startFakeControlPlane(t, () => ({
    status: 200,
    body: { skills: ['nem objeto', { id: 'uma-skill' }] },
  }));
  const source = sourceFile(t, skillSource('name: Uma Skill'));
...
test('the draft leaves a placeholder wherever a human has to decide', async (t) => {
  ...
  const output = path.join(area, 'pasta', 'nova', 'uma-skill.manifest.json');
  const source = sourceFile(
    t,
    skillSource('name: Uma Skill\ndescription: faz uma coisa', 'Rode:\n\n```bash\nnpm test\n```\n'),
  );
  ...
  assert.equal(draft.id, 'uma-skill');
  assert.equal(draft.role, 'gate', 'the role is the flag, verbatim');
  assert.equal(draft.description, 'faz uma coisa');
...
test('a source with no derivable check says so, because the registry refuses one', async (t) => {
  const plane = await startFakeControlPlane(t, emptyRegistry);
  const area = temporaryArea(t, 'cartografo-t212-draft-');
  const source = sourceFile(t, skillSource('name: sem-check', 'Só prosa, nenhum comando.\n'));
  ...
  assert.equal(
    JSON.parse(readFileSync(path.join(area, 'sem-check.manifest.json'), 'utf8')).id,
    'sem-check',
...
test('a manifest file that is not a manifest never opens a gate', async (t) => {
  ...
  await assert.rejects(
    capture(() => runProposeSkill({ path: path.join(area, 'nao-existe.json'), url: plane.url })),
    (error: unknown) => error instanceof UsageError && /could not read/.test(error.message),
  );
  await assert.rejects(
...
test('the approval is opened on the job, never auto-approvable', async (t) => {
  ...
  const area = temporaryArea(t, 'cartografo-t212-propose-');
  const manifest = path.join(area, 'uma-skill.manifest.json');
  writeFileSync(
    manifest,
    JSON.stringify({ id: 'uma-skill', origin: { repo: 'exemplo/skills', ref: 'a1b2c3d' } }),
    'utf8',
  );
...
test('a control plane that refuses the job or the approval stops the proposal', async (t) => {
  const area = temporaryArea(t, 'cartografo-t212-propose-');
  const manifest = path.join(area, 'uma-skill.json');
  writeFileSync(manifest, JSON.stringify({ id: 'uma-skill' }), 'utf8');
  ...
  const jobWithoutId = await startFakeControlPlane(t, () => ({ status: 201, body: { sem: 'id' } }));
...
const APPROVED_MANIFEST = { id: 'uma-skill', version: '0.1.0', hash: 'sha256:abc' };
...
test('a refusal is a refusal, with or without a reason written on it', async (t) => {
  const withReason = await startFakeControlPlane(t, () =>
    queue([answered('rejeitar: as instruções pedem uma credencial')]),
  );
  const first = await capture(() => runRegisterSkill({ jobId: 77, url: withReason.url }));
  assert.equal(first.code, 1);
  assert.match(first.stderr, /import refused by rafael — as instruções pedem uma credencial/);
...
test('an answer that is neither a refusal nor a manifest registers nothing', async (t) => {
  const plane = await startFakeControlPlane(t, () => queue([answered('pode subir, tá ótimo')]));
...
test('the three commands are HTTP clients and nothing else', async (t) => {
  ...
  mkdirSync(path.join(area, 'saida'), { recursive: true });
  const source = sourceFile(t, skillSource('name: uma-skill'));

  await capture(() =>
    runScanSkill(scanOptions(source, plane.url, path.join(area, 'saida', 'draft.json'))),
  );
