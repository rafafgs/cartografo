 ...
 *   `src/` builds SQL, event types and repository rows, and pointing a
 *   Portuguese-prose detector at those would be pointing it at the data format.
 *   The ten files here are the ones t180's Code Changes table names: the
 *   credential gate, the three domain validators and the five route families
 *   that answer `{erro, mensagem}`, plus the import CLI.
...
/** The surfaces t180 converts in this package: API messages and CLI output. */
...
test('t180 — no Portuguese survives in a user-facing string of packages/core', () => {
...
test('t180 — the sweep bites on real Portuguese prose', () => {
  const caught = [
    ...
    'só proposta pendente pode ser aprovada',
    'a credencial apresentada não vale mais',
    'nenhum trabalho desta execução rodou sob a versão que a proposta aplicou',
    'runner declara a propria identidade: id precisa ser uma string nao vazia',
    'o resultado desta hipótese já foi gravado',
  ];
...
test('t180 — the sweep does NOT bite on the FR2 exceptions', () => {
  const allowed = [
    ...
    '"nos" has to be a list',
    'ttl_segundos has to be a positive integer: a lease with no deadline never expires',
    'metrica_esperada has to have the shape {nome, direcao: "sobe"|"cai", de, para} for a verdict',
    'the item at position ',
    'this route requires `Authorization: Bearer <token>`',
