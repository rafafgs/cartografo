/**
 * The `cost-surveyor` command (t114, FR5).
 *
 ...
 */
export const USAGE = `usage: cost-surveyor evaluate --url <url> --execution <id> [options]
...
export async function runCli(args: string[], context: CliContext = {}): Promise<number> {
  ...
  if (subcommand !== 'evaluate') {
    process.stderr.write(`cost-surveyor: unknown subcommand: "${subcommand ?? ''}"\n${USAGE}\n`);
    return 2;
  }
  ...
  try {
    ...
    if (error instanceof UsageError) {
      process.stderr.write(`cost-surveyor: ${error.message}\n`);
      process.stderr.write('cost-surveyor: run `cost-surveyor --help` for the usage\n');
      return 2;
    }
    if (error instanceof ApiError) {
      process.stderr.write(
        `cost-surveyor: ${error.message}\n${JSON.stringify(error.body ?? null)}\n`,
      );
      return 1;
    }
    ...
    if (error instanceof TypeError) {
      process.stderr.write(`cost-surveyor: could not reach the control plane: ${error.message}\n`);
      return 1;
    }
    throw error;
  }
...
// `npx cost-surveyor evaluate --url ... --execution ...`.
