...
async function loadCli(): Promise<typeof CliModule> {
  assert.ok(
    ...
    'artifact does not exist yet: packages/cost-surveyor/src/cli.ts',
  );
...
test('t180 — --help prints the usage in English, naming the subcommand and the options', async () => {
  ...
  assert.match(USAGE, /^usage: cost-surveyor evaluate --url <url> --execution <id> \[options\]$/m);
...
test('t180 — the usage errors of evaluate are English', async () => {
  ...
  assert.equal(
    ...
    'cost-surveyor: evaluate needs --url\ncost-surveyor: run `cost-surveyor --help` for the usage\n',
  );
  ...
  assert.equal(
    ...
    'cost-surveyor: evaluate needs --execution\ncost-surveyor: run `cost-surveyor --help` for the usage\n',
  );
});
