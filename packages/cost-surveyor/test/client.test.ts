 ...
 * tests in the core.
 *
 * The same pattern as `packages/screen/src/index.ts:31`: `doFetch` is injectable
...
async function load(): Promise<typeof ClientModule> {
  assert.ok(
    ...
    'artifact does not exist yet: packages/cost-surveyor/src/client.ts',
  );
...
test('AT8 — getSessions and getJobs put execucao_id in the query when it is given', async () => {
  ...
  assert.deepEqual(
    ...
    'the trailing slash of the base url is normalized, as in packages/screen',
  );
});
