/**
 * D18 gate: no Portuguese code identifier left in `packages/cost-surveyor`
 * (t211, AT1/AT2).
 *
 * Port of `packages/screen/test/no-portuguese-identifiers.test.ts`, which is
...
/** Every scanned file, as a path relative to `packages/cost-surveyor`. */
...
test('t211 — no Portuguese identifier survives in packages/cost-surveyor/{src,test}', () => {
...
test('t211 — no Portuguese file name survives under packages/cost-surveyor/{src,test}', () => {
...
test('t211 — the sweep does NOT bite on the frozen exceptions', () => {
  const allowed = [
    ...
    '/**\n * ```\n * cost-surveyor avaliar --url http://127.0.0.1:4317 --execution 7 --tier-minimo-nos 5\n * ```\n */',
  ];
