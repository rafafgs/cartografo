...
const SCANNED_FILES = [
  ...
  path.join('bin', 'cost-surveyor.mjs'),
];
 ...
 * a bare word is the term: without it `custo` fired on this package's own name,
 * which back then still carried that word — the brand and not the lens's key
 * (D18). t303 renamed the package to `cost-surveyor` and that particular
 * collision is gone, but the boundary is the rule and not the one case.
 */
...
test('t230 — the flag sweep bites on the old spellings, and only on those', () => {
  ...
  const allowed = [
    ...
    "process.stderr.write('cost-surveyor: run `cost-surveyor --help`');",
