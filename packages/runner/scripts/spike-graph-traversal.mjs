 ...
 * `rede: {permitido: true, dominios: [...]}`, a per-domain allowlist that would
 ...
 * declares `permitido: false` (it reads a checkout and runs commands; the
...
function preflightPermissions(document, manifests) {
  ...
  for (const node of document.nos) {
    ...
    const { refusals } = resolvePermissions(resolveSkillPermissions(manifest.permissoes));
...
async function main() {
  ...
  log(`factory graph: ${document.nos.length} nodes, ${manifests.length} manifests on disk`);
  ...
  try {
    ...
    const doctoredNodes = document.nos.map((node) =>
      node.id === document.no_inicial
    ...
    const doctored = await api(
      ...
      'POST',
      '/v1/graphs',
      { ...document, classe: `${document.classe}-adulterado`, nos: doctoredNodes },
      201,
    );
    const poison = await api(
      ...
      '/v1/jobs',
      {
        ...
        no_entrada_id: document.no_inicial,
    ...
    const walked = [document.no_inicial, ...moves.map((event) => event.dados.para_no_id)];
    const gate = document.nos.find((node) => node.tipo_no === 'portao');
    ...
    console.log(`tamper refusal:     ${refusal.name} on node "${document.no_inicial}", no session opened`);
