 ...
 * explicit about who takes it: an edge's `condicao` is "a label that matches the
 * `resultado` the source node's `saida_schema` declares"
 * (`docs/spec/grafo.md`). Until this ficha nothing carried that `resultado` from
 ...
 * `passou`/`falhou`/`escalar_humano` in its own `saida` (enforced at registry
...
export interface NodeResult {
  /** The `condicao` of the edge the session says this node took. */
  resultado: string;
}
