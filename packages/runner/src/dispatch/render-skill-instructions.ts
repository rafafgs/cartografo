 ...
 * carrying since t117: "not implemented yet: rendering `instrucoes` into a
 ...
 *    `versao` is not separately checked: the registry is create-only, so an
 ...
 *    instructions render VERBATIM — `{{entrada.<caminho>}}` interpolation is
 ...
 * The block a gate is asked to emit names the `condicao` of the edges leaving
 * THIS node, taken from the graph — not the `resultado` enum of the skill's own
 * `saida`, which is a different vocabulary with different values
 * (`passou`/`falhou`/`escalar_humano`, enforced at registry entry). The graph
 * spec is explicit that an edge's label matches "the `resultado` the source
 * node's `saida_schema` declares" (`docs/spec/grafo.md`), and that schema is the
 * NODE's contract. One skill can sit under two graphs whose edges are labelled
 * differently, and reading the labels off the graph is what lets it.
 *
 * English per D18. The rendered CONTENT is Portuguese, like every other prompt
 * in this package: it IS the skill manifest, and manifests are written in
 * Portuguese (`especificacoes/formatos/exemplos/`).
 */
...
export interface RenderedSkill {
  ...
  instructions: string;
  /**
   * The policy the session runs under, resolved from `permissoes`.
   *
...
/**
 * Turns the manifest's `permissoes` into the interface's `SessionPermissions`.
 *
 * The two vocabularies are deliberately different — the manifest is a product
 * format in Portuguese, the `EngineAdapter` interface is code — and this is the
 * one place they meet. `filesystem.leitura` has no counterpart on purpose: the
 ...
 *   declared.
 */
export function resolveSkillPermissions(permissoes: unknown): SessionPermissions | undefined {
  if (!isObject(permissoes)) return undefined;

  const filesystem = isObject(permissoes.filesystem) ? permissoes.filesystem : null;
  const network = isObject(permissoes.rede) ? permissoes.rede : null;
  if (filesystem === null || network === null) return undefined;

  const allowed = network.permitido === true;
  const domains = allowed ? textList(network.dominios) : [];

  return {
    filesystem: { write: textList(filesystem.escrita) },
...
function render(resolved: ResolvedNode, skill: RegisteredSkill): string {
  ...
  const parts = [
    ...
    '---',
    '',
    `# Nó \`${node.id}\` — skill \`${skill.id}\` v${skill.versao}`,
    '',
    `Papel do nó: \`${node.papel ?? 'não declarado'}\`. Tipo: \`${node.tipo_no ?? 'não declarado'}\`.`,
    `Papel da skill: \`${skill.papel}\`. Pin: \`${skill.hash}\`.`,
    '',
    skill.descricao,
    '',
    '---',
    ...
    '',
    ...fenced('### O que entra', contract.entrada_schema ?? {}),
    ...fenced('### O que você tem que produzir', contract.saida_schema ?? {}),
    ...fenced('### Como este nó é verificado', contract.verificacoes ?? []),
    ...fenced('## Os checks declarados pela skill', skill.checks),
    ...fenced('## As permissões desta sessão', skill.permissoes),
...
export async function renderSkillInstructions(
  ...
  return {
    ...
    permissions: resolveSkillPermissions(skill.permissoes),
  };
}
