 ...
 *
 * A class per case: `POST /v1/graphs` keys the lineage by `classe`, and two
...
function traversalGraph(className: string): Record<string, unknown> {
  ...
  return { ...document, classe: className };
}
...
test("t161 — the node's skill drives the session, and the session advances the node", async (parent) => {
  ...
  await parent.test(
    ...
    async (t) => {
      ...
      assert.ok(argv.includes(gateSkill.instrucoes), "and its own skill's instructions");
    },
  );
