...
test('AT22 — the proof registers the committed factory graph and its committed skills', () => {
  ...
  const graph = JSON.parse(read(GRAPH_PATH)) as {
    arestas: Array<{ de: string; para: string; condicao: string }>;
    nos_finais: string[];
  };
  const outgoing = new Map<string, number>();
  for (const edge of graph.arestas) {
    outgoing.set(edge.de, (outgoing.get(edge.de) ?? 0) + 1);
  }
  assert.ok(
  ...
  assert.ok(
    graph.arestas.some((edge) => edge.condicao === 'retrabalho'),
