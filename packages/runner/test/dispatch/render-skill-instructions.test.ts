 ...
 * implemented yet: render `instrucoes` into a session". Three things have to be
 ...
 *   so `permissoes` stops being a document nobody reads.
 *
 * English per D18; the rendered CONTENT is Portuguese, like every other prompt
 * in this package — it stands in for the manifest, and manifests are written in
 * Portuguese.
 */
...
function registeredSkill(overrides: Record<string, unknown> = {}): Record<string, unknown> {
  return {
    ...
    hash: SKILL_HASH,
    papel: 'portao',
    descricao: 'Confere o artefato da etapa anterior com evidência própria.',
    entrada: { type: 'object', required: ['nota'] },
    saida: { type: 'object', required: ['resultado'] },
    pre_condicoes: ['a etapa anterior deixou o artefato no diretório da sessão'],
    checks: [
      {
        id: 'conferencia-com-evidencia',
        tipo: 'agentico',
        descricao: 'Portão agêntico confere com evidência própria.',
        instrucao: 'Leia o artefato e diga se ele atende ao pedido.',
        evidencia_obrigatoria: ['saida.md'],
      },
    ],
    permissoes: {
      filesystem: { leitura: ['**'], escrita: [] },
      rede: { permitido: true, dominios: ['127.0.0.1'] },
    },
    instrucoes: '# Conferir uma etapa\n\nJulgue com evidência sua, nunca com o relato de quem produziu.',
    origem: { tipo: 'nativa' },
...
function resolvedNode(edges: ResolveModule.GraphEdge[], hash = SKILL_HASH): ResolveModule.ResolvedNode {
  return {
    ...
    node: {
      ...
      papel: 'tester',
      tipo_no: 'portao',
      descricao: 'Confere o artefato e roteia.',
      skill_ref: { id: SKILL_ID, versao: '1.0.0', hash },
      contrato: {
        entrada_schema: { type: 'object', required: ['nota'] },
        saida_schema: {
          type: 'object',
          required: ['resultado'],
          properties: { resultado: { enum: ['aprovado', 'retrabalho'] } },
        },
        verificacoes: [
          { tipo: 'deterministico', comando: 'test -s saida.md', descricao: 'O artefato existe.' },
        ],
      },
    },
    edges,
...
const SINGLE_EDGE: ResolveModule.GraphEdge[] = [
  { de: 'conferir', para: 'publicar', condicao: 'sempre' },
];
const TWO_EDGES: ResolveModule.GraphEdge[] = [
  { de: 'conferir', para: 'publicar', condicao: 'aprovado' },
  { de: 'conferir', para: 'implementar', condicao: 'retrabalho' },
];
...
test('AT6 — the rendered text carries the manifest, the node contract and the escalation protocol', async () => {
  ...
  // not in the manifest's `saida` (`docs/spec/grafo.md`).
  ...
  assert.ok(text.includes('evidencia_obrigatoria'));
  assert.ok(text.includes('"escrita"') && text.includes('"permitido"'));
});
...
test('AT9 — the routing protocol is appended only when the node has more than one way out', async () => {
  ...
  // The exact `condicao` labels of THIS node, so the session is choosing from
    ...
    (
      await makeReader(
        registeredSkill({
          permissoes: {
            filesystem: { leitura: ['**'], escrita: ['**'] },
            rede: { permitido: false, dominios: ['example.com'] },
          },
        }),
      )
    ).read,
