...
function snapshot(): Record<string, unknown> {
  return {
    nos: [
      { id: 'implementar', papel: 'desenvolvedor', tipo_no: 'trabalho' },
      { id: 'conferir', papel: 'tester', tipo_no: 'portao', engine: 'codex' },
      { id: 'publicar', papel: 'deployer', tipo_no: 'trabalho' },
    ],
    arestas: [
      { de: 'implementar', para: 'conferir', condicao: 'sempre' },
...
test('AT1 — a resolvable version yields the current node and the edges leaving it', async () => {
  ...
  assert.deepEqual(
    resolved.edges.map((edge) => `${edge.condicao ?? ''}->${edge.para}`),
