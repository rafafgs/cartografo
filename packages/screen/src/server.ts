 ...
 * port, so both entry points — `npx cartografo-screen` and this module — have to
...
/**
 * Entry point of `npm start --workspace @cartografo/screen`.
 *
...
if (import.meta.filename === process.argv[1]) {
  main().catch((cause: unknown) => {
    ...
    process.stderr.write(
      `cartografo-screen: ${cause instanceof Error ? cause.message : String(cause)}\n`,
    );
  });
}
