 ...
 *
 * The screen, by contrast, comes up IN PROCESS: `bin/screen.mjs` is a thin shell
...
/** Root of the `packages/screen` package. */
 ...
 *
 * @param relatives Paths relative to the root of `packages/screen`.
 */
export function requireArtifacts(...relatives: string[]): void {
  for (const relative of relatives) {
    assert.ok(
      existsSync(path.join(PACKAGE_ROOT, relative)),
      `artifact does not exist yet: packages/screen/${relative}`,
    );
  }
}
