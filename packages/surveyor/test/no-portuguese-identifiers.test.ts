/**
 * D18 gate: no Portuguese code identifier in `packages/surveyor` (t247, AT9).
 *
 * Port of `packages/cost-surveyor/test/no-portuguese-identifiers.test.ts`,
...
const FORBIDDEN = Object.freeze([
  ...
  //
  // `topografo` itself is absent, and deliberately. It was this package's own
  // brand until t303 renamed it to `surveyor`, and it is still spelled in one
  // place the rename could not reach: `proposta-topografo.json`, the file the
  // flow lens's session writes (`runner/src/surveyor/proposal.ts`). That is an
  // artefact name agreed with the engine at run time, not an identifier — and
  // D18 moves code, never a name a person or a process types.
...
/** Every scanned file, as a path relative to `packages/surveyor`. */
function scannedFiles(): string[] {
  ...
  for (const dir of SCANNED_DIRS) {
    ...
    assert.ok(existsSync(absolute), `artifact does not exist yet: packages/surveyor/${dir}`);
    walk(absolute);
  }
  return found.sort();
}
...
test('t247 — no Portuguese identifier survives in packages/surveyor/{src,test}', () => {
...
test('t247 — no Portuguese file name survives under packages/surveyor/{src,test}', () => {
