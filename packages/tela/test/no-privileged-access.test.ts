...
test('AT16 — packages/tela declares no SQLite driver', () => {
...
test('AT17 — the gate sweep over packages/tela reports nothing', () => {
...
test('AT18 — the screen talks to the core over HTTP only, on the public /health route', async () => {
  assert.ok(
    ...
    'artifact does not exist yet: packages/tela/src/index.ts',
  );
