#!/usr/bin/env node
/**
 * The watcher's command: `cartografo-topografo`.
 *
 * A thin shell, in the same mould as `packages/topografo-custo/bin/topografo-custo.mjs`
 ...
 * `npx cartografo-topografo` does not have to know that tsx exists.
 *
 ...
 *
 * Usage: `npx cartografo-topografo watch --url <url> --token <token>`.
 */
...
try {
  ...
  console.error('cartografo-topografo: failed to run the command');
  console.error(error);
  process.exitCode = 1;
}
