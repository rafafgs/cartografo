/**
 * The watcher's command: `cartografo-topografo` (t247, FR1).
 *
 ...
 *
 * Exit codes, in the same convention as `topografo-custo` and the `cartografo`
 ...
 */
export const USAGE = `usage: cartografo-topografo watch --url <url> --token <token> [options]
...
/** The command line is wrong. Exits 2, as in `topografo-custo`. */
 ...
 *
 * The same reader `topografo-custo` uses, and for the same reason: a command
 ...
 * fallback, unlike `topografo-custo`'s. That is deliberate: this process runs
...
export async function runCli(args: string[], context: CliContext = {}): Promise<number> {
  ...
  if (subcommand !== 'watch') {
    process.stderr.write(
      `cartografo-topografo: unknown subcommand: "${subcommand ?? ''}"\n${USAGE}\n`,
    );
    return 2;
  }
  ...
  try {
    ...
    process.stderr.write(`cartografo-topografo: ${error.message}\n`);
    process.stderr.write('cartografo-topografo: run `cartografo-topografo --help` for the usage\n');
    return 2;
  }
  ...
  try {
    await runWatch({
      ...
      log: (message) => log(`cartografo-topografo: ${message}`),
