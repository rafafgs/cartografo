 ...
 *
 * Port of `packages/topografo-custo/test/no-portuguese-wire.test.ts`, narrowed
...
const SCANNED_FILES = [path.join('src', 'cli.ts'), path.join('bin', 'topografo.mjs')];
 ...
 * whole, counts. The one on the left is what keeps a bare term from firing on
 * this repository's own brand — `topografo-custo` is a package name, not the
 * cost lens's key (D18).
 */
...
test('t247 — the flag sweep bites on the old spellings, and only on those', () => {
  ...
  const allowed = [
    ...
    "process.stderr.write('cartografo-topografo: run `cartografo-topografo --help`');",
    "const { evaluateExecution } = await import('@cartografo/topografo-custo/cli');",
