 ...
 * `execution.finished` (t245) — and the real `cartografo-topografo watch`
...
function spawnWatcher(t: TestHook, plane: ControlPlane, binDir: string): Watcher {
  assert.ok(existsSync(BIN_PATH), 'artifact does not exist yet: packages/topografo/bin/topografo.mjs');
